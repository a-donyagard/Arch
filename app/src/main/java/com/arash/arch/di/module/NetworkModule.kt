...
import com.arash.arch.data.di.qualifier.WithToken
import com.arash.arch.data.di.qualifier.WithoutToken
...
import com.arash.arch.data.source.remote.LoginDataSource
import com.arash.arch.data.source.remote.TokenAuthenticator
import com.arash.arch.data.util.DataConstants
import com.arash.arch.util.SecretFields
...
object NetworkModule {
    ...
    fun provideTokenAuthenticator(
        appPreferencesHelper: Lazy<AppPreferencesHelper>,
        loginDataSource: Lazy<LoginDataSource>
    ): Authenticator {
        return TokenAuthenticator(loginDataSource, appPreferencesHelper)
    }
        ...
        builder.interceptors().add(
            Interceptor { chain ->
                ...
                val requestBuilder = request.newBuilder()
                    ...
                    .addHeader(
                        "Authorization",
                        DataConstants.tokenType + " " + preferencesHelper.getToken()
                    )
    ...
    fun provideRetrofit(
        ...
        return Retrofit.Builder().client(okHttpClient)
            ...
            .baseUrl(secretFields.getBaseUrl() + DataConstants.apiVersion)
            .build()
    }
    ...
    @Provides
    @Singleton
    fun provideLoginDataSource(@WithoutToken retrofit: Retrofit): LoginDataSource {
        return retrofit.create(LoginDataSource::class.java)
    }

    @Provides
    @Singleton
    fun provideAnimeDataSource(@WithoutToken retrofit: Retrofit): AnimeDataSource {
        return retrofit.create(AnimeDataSource::class.java)
    }
}
